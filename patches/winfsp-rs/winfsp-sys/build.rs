...
fn main() {
    ...
    if cfg!(feature = "docsrs") {
        println!(
            "cargo:warning=WinFSP does not build on any operating system but Windows. This feature is meant for docs.rs only. It will not link when compiled into a binary."
        );
    ...
    if !Path::new(&bindings_path_str).exists() {
        ...
        let bindings = bindgen::Builder::default()
            ...
            .blocklist_type(".*IO_STATUS_BLOCK.*")
            .blocklist_item(".*IO_STATUS_BLOCK.*")
